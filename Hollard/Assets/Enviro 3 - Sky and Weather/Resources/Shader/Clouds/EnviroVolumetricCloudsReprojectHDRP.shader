    	...
    	Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                {	 
                    ...
                    float originalPointDepth = Linear01Depth(LOAD_TEXTURE2D(_DownsampledDepth, i.uv * _DepthHandleScale.zw * _DepthHandleScale.xy), _ZBufferParams);
#endif
                    [unroll]
                    for (int x = -1; x <= 1; x ++) 
                    {
...
#if ENVIRO_DEPTH_BLENDING
                                float depth = Linear01Depth(LOAD_TEXTURE2D(_DownsampledDepth,uv * _DepthHandleScale.zw * _DepthHandleScale.xy), _ZBufferParams);
